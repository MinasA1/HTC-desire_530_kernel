 ...
 */
	...
	mrs	\tmp, mpidr_el1
#else
	ALT_SMP(mrc p15, 0, \tmp, c0, c0, 5)	/* MPIDR */
	ALT_UP(mov \tmp, #0)			/* for non-smp system, just give 0. */
#endif
	...
	ldr	\tmp, =mnemosyne_\footprint_str					/* Get the offset of an element */
	add	\tmp, \base, \tmp						/* Get the address of an element */
	str	\val, [\tmp]
	dsb	sy
	.endm
	...
	ldr	\tmp, =mnemosyne_\footprint_str					/* Get the offset of an element */
	add	\tmp, \base, \tmp						/* Get the address of an element */
	add	\tmp, \tmp, \idx, LSL #MNEMOSYNE_ELEMENT_SIZE_BIT_SHIFT		/* calculate data offset with cpu index first. */
	str	\val, [\tmp]
	dsb	sy
	.endm

	.macro MNEMOSYNE_GET dst, base, footprint_str
	ldr	\dst, =mnemosyne_\footprint_str					/* Get the offset of an element */
	add	\dst, \base, \dst						/* Get the address of an element */
	ldr	\dst, [\dst]
	.endm
	...
	ldr	\dst, =mnemosyne_\footprint_str					/* Get the offset of an element */
	add	\dst, \base, \dst						/* Get the address of an element */
	add	\dst, \dst, \idx, LSL #MNEMOSYNE_ELEMENT_SIZE_BIT_SHIFT		/* calculate data offset with cpu index first. */
	ldr	\dst, [\dst]
	.endm
